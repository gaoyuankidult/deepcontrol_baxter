"""
Basic PGPE for Baxter Robot, Comparing with alpha model, this one only has one layer.
"""
...
def main():
    ...
    # Make the function
    action_prediction = theano.function([input], l_action_1_formed.get_output(input))
    all_params = lasagne.layers.get_all_params(l_action_1_formed)

    # Parameters for system
    baseline = None
    num_parameters = N_INPUT_FEATURES * N_ACTIONS # five parameters
    ...
    for n in xrange(1000):
        ...
        reward1 = one_iteration(task=task, all_params=current + deltas, l_action_2_formed = l_action_1_formed, action_prediction=action_prediction)
        if reward1 > best_reward:
            best_reward = reward1
        reward2 = one_iteration(task= task, all_params=current - deltas, l_action_2_formed = l_action_1_formed, action_prediction=action_prediction)
